        ...
        Array.from(elms).forEach(function (elm) {
            elm.addEventListener(listener.on, function (domEvent) {
                ...
                if (!template.streams[event]) {
                    ...
                    template.streams[event].o.on('error', function (err) {
                        template.streams[event].i.end();
                        delete template.streams[event];
                    });
                    template.streams[event].o.on('end', function (err) {
                        template.streams[event].i.end();
                        delete template.streams[event];
                    });
                }

                template.streams[event].i.write(domEvent);
            });
        });
    }
}
