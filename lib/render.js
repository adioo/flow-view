...
function render (template_name, _options, data) {
    let self = this;
    ...
    if (!self.config.templates[template_name]) {
        return Promise.reject(new Error('Template config "' + template_name + '" not found.'));
    }

    return template(self, template_name, self.config.templates[template_name])
    .then(doRender(self, options, data));
}
